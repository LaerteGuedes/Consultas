        ...
        <div id="navbar" class="navbar-collapse collapse">
            <ul class="white nav navbar-nav {{ Request::is('dashboard') ? 'white' : '' }} {{ Request::is('/') ? 'white' : '' }}">
                ...
                @else
                    ...
                    @if(Auth::user()->role->name=="Profissional" && Auth::user()->perfil_medico_cliente == 1)
                        <li><a href="{{ route('perfil.toggle') }}">Agendar consulta</a></li>
                    @endif

                    @if(Auth::user()->role->name=="Cliente" && Auth::user()->perfil_medico_cliente == 1)
                        <li><a href="{{ route('perfil.toggle') }}">Área do Profissional</a></li>
                    @endif
                    ...
                    <li><p>Olá <strong>{{Auth::user()->name}}</strong></p></li>
